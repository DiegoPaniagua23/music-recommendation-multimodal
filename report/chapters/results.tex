...
Para la generación de recomendaciones y la evaluación del modelo, se implementó un proceso eficiente de dos etapas que desacopla el cómputo de representaciones:

\begin{enumerate}
    \item \textbf{Pre-cómputo de Embeddings (Item Tower):} Se procesan todos los ítems del catálogo a través de la \textit{Item Tower} para generar sus representaciones vectoriales estáticas. Estos embeddings se almacenan en un índice o base de datos vectorial, lo que evita re-calcular las características multimodales (audio, texto, imagen, metadatos tabulares) en cada consulta.
    \item \textbf{Recuperación (User Tower):} Dado un usuario, se procesa su secuencia histórica de interacciones a través de la \textit{User Tower} para obtener su embedding dinámico actual. Posteriormente, se calcula la similitud (producto punto) entre el embedding del usuario y los embeddings de todos los ítems indexados, recuperando los $k$ ítems con mayor puntuación de similitud.
\end{enumerate}

Este mecanismo permite realizar inferencia con baja latencia, ya que la parte más costosa computacionalmente (el procesamiento multimodal) se realiza \textit{offline}.
...
Los valores obtenidos indican que el modelo logra un desempeño competitivo. Un Recall@10 de 0.6225 significa que, en promedio, el ítem objetivo (la siguiente canción que el usuario escuchó realmente) aparece en el top-10 de recomendaciones el 62.25\% de las veces. Considerando que el espacio de búsqueda es de 10,000 ítems, este resultado es altamente significativo y valida la capacidad del modelo para reducir drásticamente el espacio de búsqueda hacia candidatos relevantes.

Por otro lado, el NDCG@10 de 0.5478 sugiere que, cuando el modelo acierta, tiende a colocar el ítem correcto en posiciones altas de la lista, aunque existe margen de mejora en el ordenamiento fino. La consistencia de las métricas a medida que aumenta $k$ (@20, @50) demuestra la robustez del sistema.

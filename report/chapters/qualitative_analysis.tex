\subsection{Análisis Cualitativo | Interpretación Fenomenológica}
...
Si bien las métricas como NDCG@10 y Recall@10 nos ofrecen una visión macroscópica del rendimiento del sistema, es imperativo descender al nivel granular para comprender la \textit{semántica} de las recomendaciones. A continuación, diseccionamos cuatro casos de estudio que revelan cómo la arquitectura \textbf{Two-Tower} negocia la tensión entre la señal colaborativa (historial de usuario), la señal de contenido (audio/texto) y los sesgos demográficos.

\subsection{Caso A: coherencia semántica y profundidad del nicho}
...
\subsection{Caso B: hegemonía del embedding demográfico}
...
\subsection{Caso C: triunfo del Contenido sobre el contexto}
...
\subsection{Caso D: alineación vocal}
...
\subsection{Discusión: mecanismo de atención de dinámica implícita}
...
\begin{enumerate}
    \item \textbf{Modo Secuencial (SASRec):} activo cuando hay repetición clara de artistas (Caso A).
    \item \textbf{Modo Contextual (Metadata):} activo cuando el historial es difuso o genérico; el modelo "rellena" los huecos con demografía (Caso B).
    \item \textbf{Modo Contenido (Audio/Visual):} activo cuando las características espectrales son salientes y distintivas, permitiendo recomendaciones \textit{cross-cultural} (Caso C).
\end{enumerate}
